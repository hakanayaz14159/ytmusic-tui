...
VLC plays through `AudioStreamProxy` on localhost. The proxy adds the CDN headers and Range requests VLC cannot send. Proxy shutdown runs on a background thread so the UI does not wait on the HTTP server poll.
